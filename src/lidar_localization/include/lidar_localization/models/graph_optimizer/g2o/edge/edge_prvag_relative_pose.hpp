...
public:
	...
	virtual void computeError() override {
		...
		// calculate value - measurement value
	...
	virtual void linearizeOplus() override{
		...
		Eigen::Vector3d delta_pji = pos_j - pos_i;

		_jacobianOplusXi.block<3, 3>(0, 0) = ori_i.matrix().transpose() * (-1);
		_jacobianOplusXi.block<3, 3>(0, 3) = ori_i.matrix().transpose() * Sophus::SO3d::hat(delta_pji);
		_jacobianOplusXi.block<3, 3>(3, 0) = Eigen::Matrix3d::Zero();
		_jacobianOplusXi.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() * (-1);


		_jacobianOplusXj.block<3, 3>(0, 0) = ori_i.matrix().transpose();
		_jacobianOplusXj.block<3, 3>(0, 3) = Eigen::Matrix3d::Zero();
		_jacobianOplusXj.block<3, 3>(3, 0) = Eigen::Matrix3d::Zero();
		_jacobianOplusXj.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity();
  	}
