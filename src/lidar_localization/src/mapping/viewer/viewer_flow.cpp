...
ViewerFlow::ViewerFlow(ros::NodeHandle& nh, std::string cloud_topic) {
    ...
    transformed_odom_sub_ptr_ = std::make_shared<OdometrySubscriber>(nh, "/lidar_odom", 100000);
    optimized_key_frames_sub_ptr_ = std::make_shared<KeyFramesSubscriber>(nh, "/optimized_key_frames", 100000);
    // publisher
    optimized_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(nh, "/optimized_odom", "/n_frame", "/lidar_frame", 100);
