...
bool LIOBackEnd::UpdateODOPreIntegration(const VelocityData &velocity_data) {
    ...
    if (!odo_pre_integrator_ptr_->IsInited() || odo_pre_integrator_ptr_->Update(velocity_data) ) 
        return true;

    return false;
}

bool LIOBackEnd::Update(const CloudData& cloud_data, const PoseData& lidar_odom, const PoseData& gnss_pose, const IMUData &imu_data) {
    ResetParam();

    if (MaybeNewKeyFrame(cloud_data, lidar_odom, gnss_pose, imu_data)) {
        AddNodeAndEdge();
        if(MaybeOptimized()) {
            SaveOptimizedPose();
        }
...
bool LIOBackEnd::MaybeNewKeyFrame(const CloudData& cloud_data, const PoseData& lidar_odom, const PoseData& gnss_odom, const IMUData& imu_data) {
    static PoseData last_lidar_pose = lidar_odom;
    ...
    if (fabs(lidar_odom.pose(0,3) - last_lidar_pose.pose(0,3)) + 
        fabs(lidar_odom.pose(1,3) - last_lidar_pose.pose(1,3)) +
        fabs(lidar_odom.pose(2,3) - last_lidar_pose.pose(2,3)) > key_frame_distance_) {
    ...
    if (has_new_key_frame_) {
        ...
        current_key_gnss_.time = current_key_frame_.time = lidar_odom.time;
        ...
        current_key_frame_.pose = lidar_odom.pose;
...
bool LIOBackEnd::AddNodeAndEdge() {
    ...
    if ( N > 1 ) {
        ...
        // add constraint, lidar frontend / loop closure detection:
        graph_optimizer_ptr_->AddPRVAGRelativePoseEdge(vertex_index_i, vertex_index_j, relative_pose, graph_optimizer_config_.odom_edge_noise);
    }
    ...
    if (graph_optimizer_config_.use_gnss) {
        ...
        graph_optimizer_ptr_->AddPRVAGPriorPoseEdge(vertex_index_j, pos, graph_optimizer_config_.gnss_noise);
    }
    ...
    if (graph_optimizer_config_.use_odo_pre_integration) {
        ...
        graph_optimizer_ptr_->AddPRVAGODOPreIntegrationEdge(vertex_index_i, vertex_index_j, odo_pre_integration_);
    }
...
bool LIOBackEnd::MaybeOptimized() {
    ...
    if (counter_.HasEnoughKeyFrames(graph_optimizer_config_.optimization_step_size.key_frame) ||
        counter_.HasEnoughLoopClosures(graph_optimizer_config_.optimization_step_size.loop_closure)) {
        need_optimize = true;
    }
...
bool LIOBackEnd::SaveOptimizedPose() {
    ...
    if (!FileManager::CreateFile(ground_truth_ofs_,   trajectory_path_ + "/ground_truth.txt")|| 
        !FileManager::CreateFile(lidar_odom_ofs_,     trajectory_path_ + "/lidar_odom.txt")  ||
    ...
    for (size_t i = 0; i < optimized_key_frames_.size(); ++i) {
        ...
        current_pose(2, 3) = 0.0f;
        SavePose(lidar_odom_ofs_, current_pose);
