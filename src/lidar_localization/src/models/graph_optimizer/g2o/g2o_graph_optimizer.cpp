...
void G2oGraphOptimizer::AddPRVAGRelativePoseEdge(
    ...
    // b.2. orientation, so3, euler angle:
    // 
    measurement.block<3, 1>(g2o::EdgePRVAGRelativePose::INDEX_R, 0) = Sophus::SO3d(
        ...
        //Eigen::Quaterniond(relative_pose.block<3, 3>(0, 0).cast<double>()).normalized().toRotationMatrix()
    ).log();
 ...
 * @return void
 */
void G2oGraphOptimizer::AddPRVAGPriorPoseEdge(
...
/**
 * @brief  add edge for odo pre-integration constraint odo ODO measurement
 * @param  vertex_index_i, vertex ID of previous key frame
 * @param  vertex_index_j, vertex ID of current key frame
 * @param  odo_pre_integration, odo pre-integration measurement
 * @return void
 */
void G2oGraphOptimizer::AddPRVAGODOPreIntegrationEdge(
    ...
    // init:
    g2o::EdgePRVAGodoPreIntegration *edge(new g2o::EdgePRVAGodoPreIntegration());
